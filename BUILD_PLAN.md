...
### 2026-09-26 — day 4: G4 wired, exam C measured, the gate rewritten red

**G4.** `screen/report.py` now prints the within-dial order of the real arms
by their trial's observed lesion ratio, gated on the S4 statistic it reads
from `results/exam_v2.json` (tau +0.80, 10 pairs, p = 0.024 today), and
never applies it to a survivor. `docs/SCREEN_RESULTS.md` regenerated; counts
unchanged, the new section is the only difference.

**Exam C** (`docs/EXAM_COCHRANE_PREREG.md`, registered before the run;
`backtest/exam_cochrane.py`): the Cochrane 2024 network RR for relapses at 24
months versus placebo, ten arms, scored on the cached curves. Every model
loses. The port on its Figure S2 readings comes closest of any scorer in the
repository — 10.0pp against 8.6pp, CI [-4.0, +13.3], two of four dial folds
under their null — and the oracle at 0.8pp says the dial mean already
predicts each arm. Numbers per model in the Result section there.

**`docs/DECISION_GATE.md`** rewritten to the measured state: red on three
models, the S2 headroom growing with the dial map (54%, 58%, 68%), S4 the one
positive result and now wired. `scripts/state_of_build.py` runs exam v2 for
every model and exam C; `results/STATE.md` regenerated by it (see the SHA it
prints).

**Definition of done, measured: red.** Per model, the number and the one
structural reason:

    Velez 2011      S1 30.7 vs 20.0   three dial patterns point the wrong way;
                                      the shared potency fits to zero
    Jenner 2026     S1 43.8 vs 30.6   time-averaged inflammation is not monotone
                                      in disease strength; two of three sign
                                      tests fail structurally
    Pernice 2020    S1 35.1 vs 19.8   signs right on the depletion, transit and
    (Figure S2)     exam C 10.0 vs 8.6 challenge dials; one shared potency cannot
                                      serve a -74% dial and a -24% dial against
                                      truths that sit between them
    Pernice 2020    S1 31.2 vs 19.8   the untreated arm saturates all 500 ODC by
    (memory read)   exam C 25.8 vs 8.6 day 310, compressing every curve

What the week bought: a model whose dials have the right sign (the port), an
exam it nearly passes (exam C), and the name of the input that is missing on
every model (per-dial magnitude from an independent source, which the MRI
channel ranks but does not size). What it did not buy: a green gate. The
plan's remaining item that a browser session on Cedric's machine would open,
the per-trial forest data behind the Cochrane bot wall, is unchanged.
