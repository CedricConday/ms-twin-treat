...
> **SPENT 2026-09-21. This file is the 2026-08-19 build handover and every
> instruction in it has been carried out or superseded. It is kept as the record
> of how the bricks were divided, not as a worklist.**
>
> What it says that is no longer true: B8 (`bricks/readout.py`) and B9
> (`bricks/vpop.py`) are marked "not built" and both shipped; the two-agent
> `cbk`/`vps` split and the `claims/<B>` protocol are not how this repo has been
> worked since; `bricks/qsp.py` is named as B4 and has been superseded for
> scoring by `bricks/qsp_velez.py`, the Vélez de Mendizábal 2011 port.
>
> **For the live state, read in this order:**
> 1. `results/STATE.md` — every gate number, regenerated by
>    `python scripts/state_of_build.py`, timestamped and stamped with the git SHA.
>    If a number in any document disagrees with that file, the document is wrong.
> 2. `BUILD_PLAN.md` §8.0 for the superseded-in-place summary table, then the
>    END of §8.4, where the dated log is. Do not read §8.1's six blockers as a
>    worklist — all six are closed or retired.
> 3. `docs/DECISION_GATE.md` if you want the short version without the code.
>
> The one-line state: the pipeline runs end to end, the screen generates and
> kills candidates and refuses to rank them, and **every out-of-sample scorer in
> this repo loses to a predict-the-mean null.** That last fact is the build, not
> a caveat on it.
